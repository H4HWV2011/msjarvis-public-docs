...
> - **P5 – Design is a geographic act** by treating routing rules, collection choices, and web‑gateway boundaries as design decisions that change how the system “knows” a region.  
> - **P12 – Intelligence with a ZIP code** by privileging West Virginia and MountainShares‑specific collections in retrieval, and by preparing the API for geography‑scoped queries even before all metadata is complete.  
...
> As such, this chapter belongs to the **Computational Instrument** tier: it defines the retrieval and routing machinery that connects Chroma‑backed semantic memory, GeoDB, and web research to the live outputs of Ms. Jarvis.
