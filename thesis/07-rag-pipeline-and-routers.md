...
``text
>- Unified Gateway → Main Brain → RAG (text + GIS + registry) → 21‑LLM ensemble → Blood–Brain Barrier

is live and serving West Virginia benefits and geography questions. A typical benefits query such as:

``bash
> curl -X POST http://127.0.0.1:8050/chat/sync \
>  -H "Content-Type: application/json" \
>  -H "X-API-Key: super-secret-key" \
>  -d '{"message":"Oak Hill WV county benefits"}'`

triggers retrieval from ChromaDB (including the `msjarvisgis` and `gis_wv_benefits` collections), synthesis by the 21‑model `llm20_production` ensemble, and post‑filtering by the guardrail service.
...
- A main brain + orchestrator that routes requests and assembles context for the 21‑model `llm20_production` ensemble.
...
curl -X POST http://127.0.0.1:8050/chat/light \
  -H "Content-Type: application/json" \
  -H "X-API-Key: super-secret-key" \
  -d '{"message":"Raleigh County WV benefits summary"}'

…which yield fast, Chroma‑conditioned responses about SNAP, Medicaid, and related programs.
