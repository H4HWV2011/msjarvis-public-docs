...
- **Record count:** 6,119,134 (includes multi‑representation geometries)  
...
A dedicated backfill process derives these extended fields from gbim_worldview_entity and gbim.gbim_attrs and attaches them to existing records in gbim_worldview_entities. Coverage is monitored via automated scripts that count non‑null values across the 6,119,134 stored records.
...
Spatial memory (gbim_worldview_entities):
  ...
  5,491,566 unique worldview entities (from public.gbim_worldview_entity)
  ...
  Extended provenance fields (worldview_id, bbox, dataset, geodb_id, srid, confidence) attached via a dedicated backfill pipeline, with coverage monitored by automated scripts
...
The conceptual role of ChromaDB as semantic memory is fully implemented for spatial entities through:
  ...
  A production gbim_worldview_entities collection with over 6.1M records indexed in a shared HTTP‑backed Chroma service
...
In consolidation:
  ...
  Extended provenance metadata (worldview_id, bbox, dataset, geodb_id, srid, confidence) is being attached via a backfill pipeline, with coverage tracked by scripted checks
