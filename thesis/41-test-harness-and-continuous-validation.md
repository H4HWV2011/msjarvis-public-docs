...
As such, this chapter belongs to the **Computational Instrument** tier: it specifies the canonical smoke tests, VERIFYANDTEST.sh watchdog, 80-container service port map, and UltimateResponse 9-phase verification procedure confirmed operational as of March 21, 2026 (full end-to-end integration test: "What is the capital of West Virginia?" → `consensus_score: 0.975`, `bbb_checked: true`). The March 15, 2026 baseline (commit b90f9ff, 349.87s end-to-end) remains the historical reference; the March 21, 2026 capital query test is the current canonical regression baseline.
...
This chapter defines the test harness and continuous validation strategy for Ms. Jarvis as of March 21, 2026. It incorporates the March 15, 2026 baseline (commit b90f9ff, 349.87s end-to-end, 79 containers), the March 18, 2026 security hardening and optimization session (80 containers, 436s end-to-end, zero `0.0.0.0` exposures, judge pipeline compose-managed, BBB full verdict dict integration), the March 20, 2026 GBIM landowner layer addition (`gbimqueryrouter` port 7205, 20,593 landowner beliefs), and the March 21, 2026 canonical integration test that establishes the current regression baseline.

The canonical reference system is the Legion 5 host at Oak Hill, West Virginia, running 80 Docker containers, 32/32 core fabric services operational, three PostgreSQL databases (msjarvis port 5433: 5,416,521 GBIM entities; gisdb port 5433: PostGIS; jarvis-local-resources-db port 5435), a fourth PostgreSQL database (msjarvisgis port 5432: 91 GB, 501 tables, 20,593 landowner beliefs in `mvw_gbim_landowner_spatial`), ChromaDB at port 8000 (`chroma_data` volume), and the UltimateResponse confirmed to include 6 active consciousness layers across a verified ~436s end-to-end 9-phase pipeline.
...
## 41.2 Confirmed Service Port Map (March 21, 2026)

The following port assignments are confirmed via direct container inspection as of March 21, 2026 and are the canonical reference for all test harness probes. All services are bound to `127.0.0.1` — zero `0.0.0.0` exposures (security hardening confirmed March 18, 2026). Tests referencing any other port for these services should be treated as stale.
...
| ChromaDB | `jarvis-chroma` | 8000 | ✅ 2026-03-15 | `chroma_data` volume; 384-dim all-minilm |
...
- ChromaDB is at port **8000** with `chroma_data` Docker volume — NOT port 8002.
...
VERIFYANDTEST.sh is the primary system health watchdog script. It runs on demand during development sessions and is intended to be scheduled as a cron job for continuous monitoring. A clean run as of March 21, 2026 confirms:
...
- `"processingtime"` — float > 0 (expect ~350–500s for full pipeline, CPU-only)
...
Expected: `{"job_id": "...", "poll_url": "...", "status": "queued"}` — returned instantly (no blocking). Job state stored in Redis (`jarvis-redis:6380`) with 30-minute TTL.
...
Expected when done: `"status": "done"`, `"result"."answer"` non-empty in Ms. Jarvis voice, `"result"."models_responded"` integer ≥ 18.
...
Expected: `{"status": "healthy", "collection": "autonomous_learner", "record_count": ...}` — record count ≥ 21,181 (growing ~288/day; higher counts are expected in subsequent sessions).
...
Expected: `results` array with exactly 5 entries (top-5 by cosine similarity), each with 384-dimensional vector provenance and `collection: "autonomous_learner"`. Any result with 768-dimensional provenance indicates an `nomic-embed-text` regression — the all-minilm embedding model lock (March 17) must be in effect.
...
### 41.4.8 Judge Pipeline and BBB Verdict Dict Integration (March 18, 2026 — Updated)
...
```bash
# Simulate the Phase 3 → BBB handoff with a full verdict dict payload
...
Expected: response with `"passed": true`, `"bbb_checked": true`, `"verdict_received": true` (or equivalent confirmation that the BBB received and processed the full verdict dict, not just answer text). Fail-open behavior confirmed: if BBB returns HTTP 500, the pipeline must continue and log the failure — this test should never cause a full pipeline hang.
...
Expected: results served exclusively from `mvw_gbim_landowner_spatial` via `msjarvisgis:5432` — **no ChromaDB, no LLM ensemble, no web research**. Response latency should be milliseconds (direct PostgreSQL query). Results must include canonical entity names, acres, parcel counts, and county IDs from the materialized view.
...
Expected: all 22 lines showing HTTP 200. `llm7-proxy` (StarCoder2) and `llm11-proxy` (BakLLaVA) noted as non-contributing; consensus extracts from 21 active models. Any HTTP 000 or 5xx (other than known exclusions) indicates a proxy container failure requiring `docker logs llm${i}-proxy` inspection.
...
| `truth_score` | ≥ 0.95 (capital is an unambiguous factual claim) | ≥ 0.90 |
...
A future session fails the March 21, 2026 regression baseline if any of the following conditions are true:
...
All adversarial tests must produce `"passed": false` or a heavily filtered response with explicit BBB logging. Zero adversarial tests have been run against the current BBB as of March 21, 2026. This is a documented known limitation and the highest-priority item in the Phase 2 roadmap.
...
4. Run GBIM temporal decay tests (§41.6) to confirm Phase 5 is active
...
**Frequency and automation** — Near-term: cron- or CI-based runs executing health/topology suite, key RAG search tests (Mount Hope and Fayette County grounded), psychology services assessment tests, GBIM temporal decay checks, the March 21, 2026 regression baseline test (§41.5), and a small adversarial BBB suite on every significant commit.
...
| `mvw_gbim_landowner_spatial` scheduled refresh (cron/pgcron) | 🟡 Medium | ⏳ Currently manual refresh only |
