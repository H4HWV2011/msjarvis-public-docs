...
H_geo is the spatial body through which Ms. Allis represents land, routes, boundaries, infrastructure, localities, and geographically grounded relationships. In Hilbert terms, it is the spatial subspace of H_App in which place-linked entities and relations are arranged so they can be compared, composed, measured, and reasoned over as part of a larger semantic whole.

H_geo is therefore not limited to geometry in the narrow sense. It includes the semantic significance of geography: counties, communities, parcels, service regions, jurisdictional layers, boundaries of authority, infrastructural adjacency, and the relationship between physical place and governed action. A coordinate becomes part of H_geo only when it is incorporated into a meaningful spatial state.
...
A spatial claim, inference, or update does not become authoritative merely because a geospatial service computed it or a reasoning cycle produced it. Candidate spatial states may be generated, compared, or tested inside the sandbox, but they remain provisional until they pass the relevant gate sequence. A staged routing inference, proposed boundary interpretation, local infrastructure linkage, or jurisdiction-sensitive conclusion remains a candidate state until the governance process permits it to cross into promoted system state.

This principle is essential because geography carries consequences. A spatial representation may affect benefits routing, infrastructure interpretation, land-linked records, local resource reasoning, flood-risk communication, or jurisdictional accountability. H_geo must therefore preserve the difference between computed possibility and governed acceptance.
...
Spatial Hilbert updates and inferences may be computed or staged through sandbox reasoning.

When Ms. Allis performs spatial reasoning, she may form candidate linkages among geometries, infrastructure records, community features, administrative layers, and spatially entangled semantic context. Those operations belong first to the sandbox domain, where reasoning remains reversible and does not automatically alter authoritative state. A sandbox cycle may generate a candidate spatial update, propose an interpretation of a place-linked condition, or assemble a new inference path across multiple spatial services.

At this stage, the output is a staged spatial state rather than a committed one. It may be held in an ephemeral reasoning structure, queued for validation, or compared against current spatial and semantic constraints. The important point is that H_geo is not authoritatively updated by the mere production of a candidate.
...
If a sandbox cycle produces a candidate spatial inference, that candidate must still satisfy the broader governed path: truth-relevant checks where applicable, guardian and constitutional requirements, provenance and coherence review, privacy and security conditions, and domain-specific spatial constraints. Only then may the candidate cross from provisional staging into promoted spatial state.

This matters because spatial knowledge often carries hidden authority. A route classification, locality linkage, parcel relation, infrastructure interpretation, or declaration of feature absence may appear technical while changing how the system understands or acts upon a place. Promotion gates therefore form the boundary between exploratory spatial reasoning and accepted spatial knowledge.

The publication manifest is one concrete expression of that boundary. A collection can exist physically without being the active governed collection. Promotion determines which physical collection represents the logical body served by retrieval.
...
H_geo is not isolated from the rest of the system. It is the spatial body that links the Hilbert architecture to GeoDB, infrastructure representations, governed belief snapshots, vector publication, and the broader place-based service environment.
...
H_geo must respect locality restriction.

Not every spatial fact should be equally visible, movable, or actionable across every part of the system. Some spatial knowledge is local by design. It belongs to a specific county, jurisdiction, census geography, neighborhood, service region, or place-bound governance context. Locality restriction means that place-linked knowledge should remain governed by its applicable locality rather than being treated as universally portable merely because it has been represented in a formal spatial structure.

This principle applies both to disclosure and to reasoning scope. A candidate spatial inference valid within one jurisdiction may not generalize beyond it. A local infrastructure pattern may be meaningful for one place while inappropriate to apply elsewhere.

The retrieval system now preserves exact locality when a query supplies a resolvable block-group identity. Enforced locality-based disclosure—the question of whether a requester is authorized to see or act upon the resulting record—remains separate and is not yet implemented as a complete retrieval condition.
...
A place is not fully represented if its governing boundaries are ignored. Counties, municipalities, census tracts, block groups, districts, service territories, state lines, and other jurisdictional or statistical divisions shape what actions are permitted, what rules apply, and how spatial conclusions should be interpreted.

This is especially important for governed-state semantics. A spatial conclusion may be geometrically plausible yet jurisdictionally invalid, or locally accurate yet institutionally inapplicable. The meaning of a spatial state therefore includes the boundary conditions under which it can be used, disclosed, or promoted.

The block-group retrieval work demonstrated that geographic identity is also hierarchical. A twelve-digit block-group GEOID contains state, county, tract, and group structure. Queries may express that identity directly or indirectly through a county FIPS code, tract identifier, and group number. H_geo must preserve that hierarchy rather than treating every identifier as an unrelated string.
...
A spatial state becomes governed when it is not only representable but also admitted under the applicable authority conditions. This includes locality fit, jurisdictional appropriateness, provenance, declared snapshot identity, publication status, cross-domain coherence where measured, and passage through the general promotion structure.

Governed-state semantics therefore distinguish among at least four spatial conditions:

- computed spatial possibility;
- staged spatial candidate;
- validated spatial publication candidate;
- promoted spatial knowledge.

This distinction became operationally important during the block-group repair. The lineage-correct candidate collection was validated before it replaced the incorrect physical collection. The prior collection was retained in quarantine rather than silently deleted. Existence, validation, and promotion were therefore separate events.
...
Because H_geo is part of H_App, spatial state must fit with the broader semantic architecture.
...
The governed-state semantics of this chapter are no longer specification alone. The West Virginia publication now exposes three geographic grains through GIS-RAG:

- **55 county documents** in the physical collection `gbimwvcountiesv2`;
...
- physical collection: `gbimwvblockgroupsv1`;
- snapshot version: `wv-blockgroup-snapshot-v1`;
- source view: `wv_blockgroup_belief_vector_ready`;
- rendering version: `blockgroup-render-v1`;
...
2. a physical representation linked to its source feature;
3. a versioned canonical belief document;
4. a deterministic SHA-256 belief hash;
5. a verified publication row;
6. a physical Chroma record with the same identity;
7. a promoted manifest entry;
8. exact or semantic retrieval through the governed service;
9. reverse trace from the retrieved record to the relational source.
...
Governed spatial belief requires the system to speak its ignorance as carefully as its knowledge.

The coverage model distinguishes among four intended states:

- **present** — features were verified at the location;
- **confirmed_none** — the source was evaluated and verified to contain no applicable feature;
- **unknown** — coverage could not be resolved;
- **missing_source** — the expected source itself is absent.

The distinction between `unknown` and `confirmed_none` carries particular epistemic weight. A failed or incomplete coverage computation must never be rendered as verified absence. Zero features under an unknown coverage state means only that no defensible feature count was established.

The authoritative block-group snapshot currently has a narrower scope than earlier drafts claimed:

- 1,639 block-group coverage rows;
- 1,639 distinct block groups;
- one represented source layer, D-FIRM/NFHL;
- zero recorded features for that source;
- coverage state `unknown` for every block group.
...
- source table `public.wv_soc_census_blockgroups_2020`;
- an embedded document consisting of four coverage-count clauses.

The authoritative relational snapshot for the same GEOID had:

- a 64-character SHA-256 belief hash;
- snapshot version `wv-blockgroup-snapshot-v1`;
...
1. Build a candidate collection from the authoritative `wv_blockgroup_belief_vector_ready` view.
2. Use the same `all-minilm:latest` embedding model and 384-dimensional space used by GIS-RAG.
3. Confirm 1,639 candidate records and exact agreement between candidate identifiers and authoritative SHA-256 belief hashes.
4. Validate deterministic metadata lookup and preserve ordinary semantic retrieval.
5. Quarantine the prior physical collection as `gbimwvblockgroupsv1quarantine20260714`.
6. Rebuild `gbimwvblockgroupsv1` from the lineage-correct candidate.
7. Restore GIS-RAG to the governed physical name.
8. Re-run exact retrieval, semantic-mode, count, manifest, and lineage tests.

The repaired physical collection contains exactly 1,639 records and its identifier set matches the manifest publication set. Its collection hash is:
...
This incident establishes an important invariant for H_geo:

> Equal record counts do not establish equal governed state.
...
GIS-RAG was therefore upgraded to hybrid retrieval:

- exact twelve-digit block-group GEOIDs are recognized;
...
- direct block-group GEOID;
- tract followed by group;
- group followed by tract;
- decimal tract with county FIPS and group;
- explicit request field.

Ordinary semantic retrieval remained active for non-identifier queries.

This division is not a workaround around semantic retrieval. It is the correct mathematical and epistemic separation of functions:
...
\text{query}
\rightarrow
\text{canonical block-group identity}
\rightarrow
\text{Chroma document}
\rightarrow
\text{belief hash}
\rightarrow
\text{verified publication row}
\rightarrow
\text{belief snapshot}
\rightarrow
\text{representation}
\rightarrow
\text{source feature }1295
\]
...
A separate block-group defect belongs in this chapter because it concerns the spatial substrate itself.

The representation table had previously carried multiple competing geometry columns, including a column whose declared geographic reference did not match its projected coordinate values and an unindexed duplicate of the authoritative projected geometry. A default-looking column that returns projected meters while claiming geographic degrees is especially dangerous because transformations and joins can complete without an obvious runtime failure while producing incorrect spatial results.

The remediation was gated:

1. establish whether the competing geometries were spatially equivalent;
2. identify scripts and views referencing each column;
3. repoint authoritative paths to the correct projected geometry;
4. remove the unindexed duplicate;
5. remove the mislabeled column only after remaining dependencies would fail loudly.
...
- A new Phi coherence reading against the repaired canonical block-group collection. The earlier three-grain reading included the superseded split-brain collection and must not be treated as the canonical baseline.
...
Spatial updates and inferences acquire authority only through an inspectable path of identity, representation, canonical belief, deterministic hashing, verified publication, promotion, retrieval, and reverse trace. Exact place identifiers are resolved deterministically; semantic spatial questions remain available to vector retrieval. These are complementary operations rather than competing theories of spatial meaning.
