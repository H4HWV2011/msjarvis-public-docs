...
a physical representation linked to its source feature;
...
Build a candidate collection from the authoritative wv_blockgroup_belief_vector_ready view.
...
query → canonical block-group identity → Chroma document → belief hash → verified publication row → belief snapshot → representation → source feature 1295.
...
establish whether the competing geometries were spatially equivalent;
identify scripts and views referencing each column;
repoint authoritative paths to the correct projected geometry;
