...
> | `ms_allis_memory` records | ≥ 296 | `preflight_gate.sh` |
> | GBIM entities | ≥ 5,416,521 | `preflight_gate.sh` |
> | msallisgis public tables | 515 | `SELECT count(*) FROM pg_tables WHERE schemaname='public'` |
> | msallisgis total tables | 742 | `SELECT count(*) FROM pg_tables` |
> | msallisgis size | 95 GB | `SELECT pg_size_pretty(pg_database_size('msallisgis'))` |
...
- **Ingest events** — documents arriving through `allis-ingest-api` and `allis-ingest-watcher` (AAPCAppE pipeline and similar external-source feeds).
...
The `Authorization: Bearer $ALLIS_API_KEY` header is required. Requests without a valid key are rejected before first-stage evaluation begins.

### Ingest Path

External documents arrive through `allis-ingest-api` and `allis-ingest-watcher`. These are evaluated for domain relevance and structural quality before being written to `msallis_docs` or `autonomous_learner`. This path bypasses the LLM ensemble — the first-stage gate for ingest events is domain-relevance scoring, not BBB truth filtering.
...
When a `/chat` call completes, the `background_rag_store` task writes a new record to `ms_allis_memory` (UUID: `79240788-0828-45f3-b1bc-a9a3593628a6`) via this binding. Every successful interaction produces exactly one new background record. The collection currently holds 296 records and must be verified by UUID — see Ch. 22 §22.10 for UUID preservation risk.
...
### msallisgis (port 5435)

The authoritative GBIM and GIS store. All GBIM data is in `msallisgis` on port 5435 — not on port 5433. As of April 13, 2026:

| Metric | Value | Command |
|---|---|---|
| Public tables | 515 | `SELECT count(*) FROM pg_tables WHERE schemaname='public'` |
| Total tables | 742 | `SELECT count(*) FROM pg_tables` |
| Database size | 95 GB | `SELECT pg_size_pretty(pg_database_size('msallisgis'))` |
...
### allis-msallis-db (port 5433)

Added to `docker-compose.yml` on April 13, 2026 with `external: true` volume declaration for `msallis_pgdata`. Container is confirmed healthy. The prior `msallis-pgdata-rescue` container (wrong volume, 37 rows only, `gbim_entities` schema present with 0 live rows) is retired. The `msallis` database on port 5433 is currently offline for live GBIM operations — all live GBIM data is in `msallisgis` on port 5435.
...
1. **Authentication** — `ALLIS_API_KEY` must be present and valid. No evaluation proceeds without it.
...
6. **Background write** — If all stages complete, `background_rag_store` writes one record to `ms_allis_memory` and the BBB audit event is persisted to `allis-memory:8056` via `/memory/turn`.
...
2. **Direct write** — Documents passing relevance scoring are written directly to `msallis_docs` or `autonomous_learner` with metadata `services: ["aappcappe-ingest"]` and `type: "community_resource"`. No LLM ensemble or BBB filtering is applied to ingest events.
...
- Produce final GBIM updates — first-stage outputs are background store candidates. Promotion to GBIM tables in `msallisgis:5435` is a human-guided or consolidation-pass operation (see Ch. 21 §21.6 and Ch. 22).
...
Must pass all 9 checks before any `docker compose up`. Canonical source of truth for collection counts, key collection record counts, GBIM entity counts, and `allis-memory` auth health.
...
| ChromaDB | 8002 | 8000 | `allis-chroma`; `/api/v2/` only |
...
| msallisgis (GBIM + GIS) | 5435 | 5432 | `allis-local-resources-db`; authoritative GBIM |
| allis-msallis-db | 5433 | 5432 | External volume `msallis_pgdata`; offline for live GBIM |
