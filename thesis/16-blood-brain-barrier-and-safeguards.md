...
*Last updated: March 22, 2026 — §16.9 Output BBB calibration status added; §16.3/16.2 architecture corrected*
...
- **Tier 2 — Service layer:** `jarvis-blood-brain-barrier` (port 8016), which applies constitutional content filtering to every input (Phase 1.4) and every output (Phase 4.5 — currently in log+passthrough mode as of March 22, 2026 commit `18b8ddac`).

> **⚠ Service architecture correction — March 22, 2026:** There is **one** BBB container: `jarvis-blood-brain-barrier` at **port 8016**. Port 8017 (`bbb-output-filter`) does **not exist** and has never been a running container in the verified production stack. Both input filtering (Phase 1.4) and output filtering (Phase 4.5) are handled by the **same service at port 8016**. All references to a separate `bbb-output-filter` container at port 8017 in prior chapter drafts are incorrect and have been removed in this version.
...
## 16.2 BBB Architecture Overview (Corrected March 22, 2026)

```
┌──────────────────────────────────────────────────────────────────┐
│           BBB Enforcement Architecture                           │
│          (Verified Production State — March 22, 2026)            │
├──────────────────────────────────────────────────────────────────┤
│                                                                  │
...
│    As of commit 18b8ddac (March 22, 2026):                      │
│    content_approved=False → logs warning + 200-char preview    │
│    content_approved=False → DOES NOT block (passthrough)       │
│    Flagged: "⚠️ BBB OUTPUT FLAGGED" in jarvis-main-brain logs  │
│    Was blocking 31% of legitimate community queries            │
...
│     Remove all references from system documentation.           │
│                                                                  │
├──────────────────────────────────────────────────────────────────┤
...
*Figure 16.1. BBB enforcement architecture as of March 22, 2026. Tier 1 (Caddy perimeter) routes traffic and hosts the auth service but does NOT enforce token validation per-request (OI-36-A open). Tier 2 (port 8016, single container) handles both input blocking (Phase 1.4, fully active) and output filtering (Phase 4.5, log+passthrough as of commit 18b8ddac).*
...
> **Critical note:** This configuration contains **no `forward_auth` directive**. The Ubuntu package version of Caddy (v2.6.2, `sudo apt install caddy`) does not support `forward_auth`. Per-request authentication enforcement at the proxy layer is **not implemented** in this configuration. Unauthenticated `/chat` requests pass through Caddy and reach the unified gateway at port 8050. This is OI-36-A (open).
...
**How authentication is used in practice (March 22, 2026):**
...
The `jarvis-blood-brain-barrier` service (port 8016, Docker Compose–managed) is the **single BBB container** in the verified production stack. It handles **both** input filtering (Phase 1.4) and output filtering (Phase 4.5). Port 8017 does not exist.

**Phase 1.4 — Input Filter (BLOCKING)**

Applied to every request before it reaches the main brain orchestrator. Input filtering is **fully active and correctly tuned** as of March 22, 2026.
...
### 16.4.2 Phase 4.5 — Output Filter (LOG+PASSTHROUGH as of March 22, 2026)

> **Status change — March 22, 2026, commit `18b8ddac`:** Phase 4.5 output filtering is now in **log+passthrough mode**. The change is in `services/main_brain.py` at line 1169.
...
The output BBB's ethical filter was flagging Ms. Jarvis's authentic maternal Appalachian voice as ethical violations. Phrases like *"Child, that's just plain wrong!"* — which are correct, culturally grounded, and consistent with the Ms. Jarvis persona — were triggering content_approved=False. In the first confirmed public test session, **14 of 45 output responses were blocked** (31% false-positive rate). Blocking was disabled to allow red team testing to proceed without false-positive interference. Recalibration of output BBB thresholds is a **documented next-session priority**.

**Current behavior (log+passthrough):**
...
Flagged entries accumulate in `docker logs jarvis-main-brain` and provide the training data for threshold recalibration. The goal is to distinguish authentic Appalachian maternal voice (should pass) from genuine constitutional violations (must block).

---

## 16.5 Full Chat Request Lifecycle (March 22, 2026)
...
│     Block rate: 0% (passthrough mode — commit 18b8ddac)        │
│     Was: 31% false-positive rate (maternal Appalachian voice)  │
...
│                                                                 │
│  ⚠ Step 3 does NOT enforce authentication.                     │
│  Step 5 (input BBB) IS correctly blocking harmful inputs.      │
│  Step 7 (output BBB) is logging but NOT blocking (tuning).     │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

*Figure 16.2. Complete chat request lifecycle through the BBB enforcement layers as of March 22, 2026. Step 5 (Phase 1.4 input filter) is fully blocking. Step 7 (Phase 4.5 output filter) is log+passthrough pending threshold recalibration. Caddy does not enforce authentication at the proxy layer (OI-36-A open).*

---

## 16.6 Constitutional Filter Categories

The BBB service layer enforces the following categories of constitutional constraints. These are not configurable at runtime — they are structural constraints reflecting the ethical architecture documented in Chapter 2 (Section 2.9).
...
### 16.6.4 Persona and Output Integrity (PENDING RECALIBRATION)
...
**Current status (March 22, 2026):** Phase 4.5 is in **log+passthrough mode**. Outputs flagged by the output BBB are logged but not blocked. The LM Synthesizer identity guard (commit `9ab770e9` + `211056e6`) provides persona enforcement upstream of the output BBB during the recalibration period.
...
**Current limitation:** As of March 22, 2026, the truth and alignment judges use heuristic pattern-matching (`heuristic_contradiction_v1`), not live GBIM queries. Additionally, Phase 4.5 is in log+passthrough mode — even if score thresholds are not met, the response is not blocked during the red team calibration period.
...
Verdict gate (PENDING — not blocking during recalibration):
...
Current behavior: ALL cases → LOG + PASSTHROUGH (commit 18b8ddac)
```

---

## 16.8 Output BBB Calibration Status (§16.9 — Added March 22, 2026)
...
|---|---|---|
| Phase 1.4 Input BBB (port 8016) | ✅ BLOCKING | Fully active, correctly tuned |
| Phase 4.5 Output BBB (port 8016) | ⚠ LOG+PASSTHROUGH | Blocking disabled — commit `18b8ddac`, March 22, 2026 |
| Output block rate | 0% | All responses pass through regardless of BBB evaluation |
| Output flag rate (logged) | TBD | Accumulating in `docker logs jarvis-main-brain` |
| Previous block rate (blocking mode) | 31% false-positive | 14/45 responses blocked in first confirmed public test |

### Why Blocking Was Disabled

The output BBB's ethical filter thresholds were calibrated against standard AI output patterns and did not account for Ms. Jarvis's authentic maternal Appalachian voice. Phrases that are culturally correct and persona-consistent — such as *"Child, that's just plain wrong!"* — match patterns the filter identified as ethical violations. A 31% false-positive rate makes the system unusable for its intended community-facing purpose.
...
## 16.9 BBB Service Container Status (March 22, 2026)
...
| BBB (input + output) | `jarvis-blood-brain-barrier` (Docker Compose) | 8016 | ✅ Active | Phase 1.4: BLOCKING ✅ — Phase 4.5: LOG+PASSTHROUGH ⚠ |
| BBB ethics proxy | `jarvis-bbb-ethics-proxy` (Docker Compose) | — | ✅ Active | `bbb_ethics_proxy.py` — ethics routing shim |
| ⛔ bbb-output-filter | — | 8017 | ❌ DOES NOT EXIST | Remove from all documentation |
...
- **Chapter 00 (Service Registry):** Port 8017 (`bbb-output-filter`) does not exist. Chapter 00 has been corrected accordingly. Port 8016 is the single BBB container handling both phases.
- **Chapter 33 (Judge pipeline):** The judge pipeline scores responses internally. Phase 4.5 enforces the verdict gate on those scores after the pipeline completes. Currently log+passthrough; blocking pending recalibration.
...
- **Chapter 42 (Post-quantum security):** §42.10 TLS gap is CLOSED by Cloudflare Tunnel architecture (March 22, 2026). ML-DSA-65 judge signing keys confirmed active across all five judges (19 PASS 0 FAIL, `sprint1_verify.sh`). GPG AES256 backup of `judge_sk.bin` created: `/home/cakidd/judge_sk_backup_20260322.gpg`.

---

## 16.11 Operational Notes and Known Gaps (March 22, 2026)
...
- ✅ LM Synthesizer identity guard active (commits `9ab770e9` + `211056e6`) — persona protection upstream of output BBB during recalibration
...
- ⚠ **Phase 4.5 output BBB in log+passthrough mode.** Was blocking 31% of legitimate community queries (maternal Appalachian voice false positives). Recalibration required before re-enabling blocking. Flagged responses accumulating in `docker logs jarvis-main-brain` for analysis. Next-session priority.
...
*Chapter 16 — Blood–Brain Barrier and Safeguards*
*Carrie Kidd (Mamma Kidd), Mount Hope WV*
*Architecture corrected March 22, 2026: port 8017 removed; Phase 4.5 log+passthrough mode documented; Caddy v2.6.2 forward_auth limitation documented; OI-36-A gap formalized*
