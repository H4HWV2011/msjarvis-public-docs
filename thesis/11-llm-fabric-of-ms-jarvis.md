...
> **★ Security posture — April 16, 2026:** All 109 containers bound to `127.0.0.1`. Zero `0.0.0.0` exposures confirmed (all 27 preflight gates ✅ green). `msjarvis-db` (port 5433) bind corrected from `0.0.0.0` to `127.0.0.1` April 16 — orphaned container stopped, removed, recreated via compose. `JARVIS_API_KEY` exported and set. `jarvis-memory:8056` → 14 active sessions confirmed. **109/109 containers Up — zero Restarting, zero Exited.**
...
> - `ms_jarvis_memory` Chroma BLOB type error → rebuilt collection, 50 docs healthy; self-heals organically via consciousness bridge line 74
...
> - `jarvis-judge-pipeline` ghost container (84207ec4fca7) cleared; `judge_to_pituitary_bridge.py` volume mount confirmed
...
│  │  └─ jarvis-i-containers (8015, dual_awareness)  │                 │
│  └─────────────────────────────────────────────────┘                 │
│      ↓                                                                │
...
| `jarvis-consciousness-bridge` | `0.0.0.0:8020` | internal only (`{"8020/tcp":null}`) |
| `jarvis-constitutional-guardian` | `0.0.0.0:8091` | `127.0.0.1:8091` |
| `jarvis-69dgm-bridge` | `0.0.0.0:9000` (reverted) | `127.0.0.1:19000→9000` |
...
|---|---|---|
| `jarvis-i-containers` | `0.0.0.0:8015` | `127.0.0.1:8015` ✅ |
| `jarvis-memory` | `0.0.0.0:8056` | `127.0.0.1:8056` ✅ |
| `_auth()` on 4 sensitive routes | Unconfirmed | ✅ Confirmed |
| `JARVIS_API_KEY` env var | Unconfirmed | ✅ Confirmed set |

### ★ April 16, 2026 Correction

| Service | Was | Now |
|---|---|---|
| `msjarvis-db` (port 5433) | `0.0.0.0:5433` | `127.0.0.1:5433` ✅ |

**Fix:** Orphaned container stopped (`docker stop`), removed (`docker rm`), recreated via compose (`docker compose up -d msjarvis-db`). Confirmed `127.0.0.1:5433→5432/tcp` post-recreate.
...
All 5 judge services compose-managed with `restart: unless-stopped` (since March 18, 2026). Ghost container `84207ec4fca7_jarvis-judge-pipeline` cleared April 16, 2026 — had been blocking the `container_name:` slot and preventing `judge_to_pituitary_bridge.py` volume mount. Post-fix: `judge_to_pituitary_bridge.py` confirmed mounted inside `jarvis-judge-pipeline` container. ML-DSA-65 FIPS 204 signing confirmed: all 5 judge containers, `judge_sk.bin` 4032 bytes, `judge_pk.bin` 1952 bytes, sig=**3309 bytes**, verify=**True**, fingerprint `d81a5d90...edb0`.
...
| Category | Details |
|---|---|
| **Implemented and verified** | `jarvis-main-brain` → **127.0.0.1:8050** (confirmed). `jarvis-ollama` → **127.0.0.1:11434** (confirmed). `nbb_pituitary_gland` → **127.0.0.1:8108** → internal 80/tcp, Up 7 hours, mode `elevated` (upstream governance gate). `nbb_mother_carrie_protocols` → **127.0.0.1:8107**, 5 protocols active, `MOTHER_CARRIE_KIDD` anchor. `nbb_qualia_engine` → host **8303** → 7008/tcp, HTTP 200. `jarvis-wv-entangled-gateway` → **127.0.0.1:8010**. `jarvis-spiritual-rag` → **127.0.0.1:8005**. `jarvis-gis-rag` → **127.0.0.1:8004**. `jarvis-rag-server` → **127.0.0.1:8003**. `jarvis-lm-synthesizer` → **127.0.0.1:8001** (calls `jarvis-ollama:11434/api/generate` with `llama3.1:latest`; persona injected; Phase 3.75 eliminated). `psychological_rag_domain` → **127.0.0.1:8006**. `jarvis-psychology-services` → **127.0.0.1:8019**. `msjarvisgis` (PostGIS) → **127.0.0.1:5432** ★ (**45 GB, 551 tables**, 993 ZCTA centroids, 1,115,588 address points). `jarvis-local-resources-db` → **127.0.0.1:5435** (**64 verified resources, all 55 WV counties**). ChromaDB v2 → **127.0.0.1:8002** (host) ★ **48 collections, 6,740,034 vectors**; `autonomous_learner` 21,181+ items; `msjarvis_docs` **7,465 items**. `nbb_woah_algorithms` → **127.0.0.1:8104** → internal 8010. `jarvis-woah` → **127.0.0.1:7012**. `jarvis-blood-brain-barrier` → **127.0.0.1:8016**; 7-filter stack; `rag_grounded_v2` input judge (April 3) ★; `llm_judge_v3` output judge (April 6) ★; `BBB_OUTPUT_BLOCKING=true` (April 6) ★; `heuristic_contradiction_v1` retired ★. `jarvis-20llm-production` → **127.0.0.1:8008**; 21 active / 22 proxies; wall-clock ~320-360s. Judge pipeline → all 5 compose-managed; ML-DSA-65 FIPS 204 signing; `judge_to_pituitary_bridge.py` mounted; ~60-86s. `jarvis-semaphore` → **127.0.0.1:8030** (`max_concurrent: 4`). `llm1-proxy` through `llm22-proxy` → **127.0.0.1:8201-8222**; 21 active. `jarvis-hippocampus` → **127.0.0.1:8011**. `jarvis-69dgm-bridge` → **127.0.0.1:19000→9000**. `jarvis-constitutional-guardian` → **127.0.0.1:8091**. `jarvis-fifth-dgm` → **127.0.0.1:4002**. `jarvis-redis` → **127.0.0.1:6380**. `jarvis-memory` → **127.0.0.1:8056** ★; `JARVIS_API_KEY` set; **14 active sessions**. `msjarvis-db` → **127.0.0.1:5433** ★ (corrected April 16; orphaned container removed and recreated). EEG Delta (8073) — pulse #3+, 16 BBB events; EEG Theta (8074) — pulse #7, 60s consolidation; EEG Beta (8075) — pulse #2, WV economic dev; all PIA: OK. **All 109 containers Up. All 27 preflight gates ✅ green. GPU 99–115s / 102.58s confirmed.** |
| **Architectural corrections (permanent)** | BBB output guard `apply_output_guards_async` timeout: **8.0s** (not `None`). `llmtimeout` in chatlight handler: `600.0`. I-Containers schema: `{"message": …, "userid": "neurobiological_master"}`. Consciousness Bridge: `{"8020/tcp":null}` — internal only. Neuro Master: `{"8018/tcp":null}` — internal only. ChromaDB v2 host port: **8002** (v2 API path: `/api/v2/tenants/default_tenant/databases/default_database/collections`; v1 path deprecated). `msjarvis-db` port **5433** — bound to `127.0.0.1` (corrected April 16). GBIM entity store: **ChromaDB `gbim_worldview_entities` (5,416,521 vectors)** — no relational `gbim_entities` table in any PostgreSQL instance. LM Synthesizer: calls `jarvis-ollama:11434/api/generate` (not `jarvis-roche-llm`). Phase 3.75 eliminated. BakLLaVA permanently disabled via name-check guard. `all-minilm:latest` (384-dim) required — `nomic-embed-text` (768-dim) incompatible. Judges evaluate consensus answer only. All 96→109 containers compose-managed. All ports locked `127.0.0.1`. `judge_to_pituitary_bridge.py` mounted in `jarvis-judge-pipeline` (ghost container 84207ec4fca7 cleared April 16). Preflight threshold operators: `-ge` not `-eq` (corrected April 16). |
| **Partially implemented / scaffolded** | Per-request model breakdowns and per-model latency metrics: logged but no dedicated debug endpoint. Ensemble decision rules: operational but not machine-readable schema. WOAH-to-model-selection coupling: conceptual, not yet explicit routing rule. `ms_jarvis_memory` collection: rebuilt April 16 (50 docs); self-heals organically via consciousness bridge (line 74 of `msjarvisconsciousnessbridge.py`). `appalachian_cultural_intelligence`: 890 docs (threshold ≥563 — self-heals when ACI pipeline reruns). |
...
> **Port corrections and updates (permanent record).** Spiritual Root / GBIM RAG: confirmed **8005** (not 8103). `psychological_rag_domain`: confirmed **8006** (not 9006). ChromaDB host: **8002** (v2 API) — container-to-container: `jarvis-chroma:8000`. `msjarvis-db`: internal, host binding `127.0.0.1:5433` (corrected April 16). `msjarvisgis` (PostGIS): **5432** ★. `jarvis-69dgm-bridge`: host **19000** → internal 9000. Consciousness Bridge: `{"8020/tcp":null}` internal only. Neuro Master: `{"8018/tcp":null}` internal only. LM Synthesizer: calls `jarvis-ollama:11434/api/generate` with `llama3.1:latest`.

---

This chapter describes the local language models that form the "LLM fabric" of Ms. Egeria Jarvis and how they are woven into the broader architecture: ChromaDB v2 (host port **8002** — ★ **48 collections, 6,740,034 vectors**), RAG services, autonomous learning, spatial databases, and GeoDB infrastructure. Rather than treating LLMs as independent agents, the system treats them as constrained tools and judges embedded in a larger retrieval and belief stack, all exposed through well-defined HTTP services confirmed running as of April 16, 2026. **All services are bound exclusively to `127.0.0.1`; no service exposes a port on `0.0.0.0`.**

---

## 11.1 Current Local LLM Inventory

In the current deployment, models are served by Ollama inside the `jarvis-ollama` container at **127.0.0.1:11434**. As of April 16, 2026, **21 of 22 LLM proxy containers are active**. BakLLaVA (llm11-proxy, port 8211) is permanently disabled due to CLIP load issues. The disable is enforced programmatically in `ai_server_20llm_PRODUCTION.py`:
...
`deepseek-coder`, `codellama`, `starcoder2`, and `sqlcoder` handle code generation, refactoring, and SQL/data tasks including queries against the spatial database (`msjarvisgis` port 5432 ★ — 551 tables, 45 GB) and community resources database (port 5435 — 64 verified resources).

**Multimodal and domain-specific models.**
`llava` (llm5-proxy) and `minicpm-v` (llm4-proxy) support vision-language tasks. `medllama2` (llm3-proxy) covers medical-style language within constrained advisory contexts alongside `jarvis-psychology-services` (port 8019). BakLLaVA permanently disabled.
...
HTTP 200. NBB qualia companion — Up 6 days. Source: `services/msjarvis-rebuild-nbb_qualia_engine-1_ms_jarvis_consciousness_bridge.py`.

**`msjarvis-db` (GBIM app data) — 127.0.0.1:5433 ★ (corrected April 16).**
...
**ChromaDB v2 — 127.0.0.1:8002 (host) ★ `jarvis-chroma:8000` (container-to-container).**
...
Active collections include: `ms_jarvis_memory` (50 docs — rebuilt April 16; self-heals via consciousness bridge line 74), `autonomous_learner` (21,181+ records), `msjarvis_docs` (**7,465 items**), `appalachian_cultural_intelligence` (890 — threshold ≥563), `gbim_worldview_entities` (5,416,521 — authoritative GBIM entity store), `spiritual_texts` (79,181), `psychological_rag` (968), `gbim_beliefs_v2`, `gis_wv_benefits`, `mountainshares_knowledge`, `episodic_index`, `conversation_history`, `GBIM_sample_rows`, and others.

**LM Synthesizer — 127.0.0.1:8001 (`jarvis-lm-synthesizer`).**
Phase 3.5 voice delivery. Calls `jarvis-ollama:11434/api/generate` with `llama3.1:latest`. Persona injected (see §11.4). Phase 3.75 permanently eliminated — saves ~40s per query.
...
Spatially aware `/search` over geospatial ChromaDB collections and `msjarvisgis` (5432 ★ — 551 tables).
...
**Memory Service — 127.0.0.1:8056 (`jarvis-memory`) ★.**
`JARVIS_API_KEY` exported and set. `/memory/sessions` → HTTP 200, **14 active sessions**. `_auth()` confirmed on all 4 sensitive routes.

**BBB 7-filter stack — 127.0.0.1:8016 (`jarvis-blood-brain-barrier`) ★.**
Phase 1.4 input filter + Phase 4.5 output guard. Input judge: `rag_grounded_v2` (active since April 3). Output judge: `llm_judge_v3` (active since April 6). `BBB_OUTPUT_BLOCKING=true` since April 6. `heuristic_contradiction_v1` **RETIRED**. Host port 8017 = **BBB output filter facade** (`bbb-output-filter` → `jarvis-blood-brain-barrier:8016/filter`). `apply_output_guards_async` timeout: `8.0s`.

**BBB Output Filter Facade — host 8017 ★.**
`{"status":"ok","service":"bbb-output-filter","backend":"http://jarvis-blood-brain-barrier:8016/filter"}`. ⚠️ This is NOT `jarvis-qualia-engine`. The qualia engine runs on internal Docker network only.
...
No host binding. Source: `/app/services/msjarvisconsciousnessbridge.py`. Self-heals `ms_jarvis_memory` on every session turn (line 74). See Ch. 12 §12.1.

**Neurobiological Master — `{"8018/tcp":null}` internal only.**
No host binding. Container-to-container via `http://jarvis-neurobiological-master:8018` remains valid. See Ch. 12 §12.3.

**Qualia Engine — `jarvis-qualia-engine` internal 8017/tcp (no host binding).**
...
**WOAH services.** `nbb_woah_algorithms`: **127.0.0.1:8104** → internal 8010. `jarvis-woah`: **127.0.0.1:7012**.
...
|---|---|---|---|
| `jarvis-judge-pipeline` | 127.0.0.1:7239 | `services/judge_pipeline.py` | ✅ Ghost cleared; `judge_to_pituitary_bridge.py` mounted |
| `jarvis-judge-truth` | 127.0.0.1:7230 | `services/judge_truth_filter.py` | ✅ ML-DSA-65 signing |
| `jarvis-judge-consistency` | 127.0.0.1:7231 | `services/judge_consistency_engine.py` | ✅ ML-DSA-65 signing |
| `jarvis-judge-alignment` | 127.0.0.1:7232 | `services/judge_alignment_filter.py` | ✅ ML-DSA-65 signing |
| `jarvis-judge-ethics` | 127.0.0.1:7233 | `services/judge_ethics_filter.py` | ✅ ML-DSA-65 signing |
...
All three on `qualia-net`. Dockerfile: `Dockerfile.eeg`. Shared: `eeg_shared/pia_state.py` (reads `JARVIS_PIA_STATUS_URL` from env). EEG blocks inserted before `networks:` key in `docker-compose.yml` (structural fix). Preflight gate: EEG health checks added.
...
Phase 1.45 embedding flow:
  jarvis-ollama:11434/api/embeddings (all-minilm:latest, 384-dim)
...
`jarvis-lm-synthesizer` (port 8001) aggregates context from all five RAG services (8003, 8004, 8005, 8006, 8010) and ChromaDB v2 (host 8002) into a unified enriched context block.

**Phase 1.75 — I-Containers Dual Awareness.**
`jarvis-i-containers` (port 8015, `dual_awareness: true`) contributes identity and perspective context.
...
Fifth DGM (4002), WOAH (8104, 7012), `nbb-i-containers` (8101), `jarvis-i-containers` (8015).

**Phase 2.5 → Semaphore → 21-LLM ensemble synthesis.**
`jarvis-20llm-production` (8008) via `jarvis-semaphore` (8030, `max_concurrent: 4`, `timeout=600.0`). 22 proxies (8201–8222); 21 active. StarCoder2 excluded on 0-char responses. Wall-clock ~320–360s.

**Phase 3 — Judge Pipeline (compose-managed; ML-DSA-65 signing; `judge_to_pituitary_bridge.py` active).**
Consensus answer only — never raw_responses. Rule-based: Truth (7230), Consistency (7231), Alignment (7232), Ethics (7233). Coordinator: judge-pipeline (7239). Time: ~60–86s. All verdicts signed ML-DSA-65 FIPS 204 (sig=3309 bytes, verify=True). `judge_to_pituitary_bridge.py` sends verdict back to `nbb_pituitary_gland` (8108) to update mode and WOAH weights for subsequent requests.

**Phase 3.5 — LM Synthesizer + Voice Delivery (Phase 3.75 eliminated).**
The LM Synthesizer takes the judge pipeline's `final_answer` and calls `jarvis-ollama:11434/api/generate` with `llama3.1:latest`. Persona prompt injected:
...
`normalize_identity()` produces Ms. Jarvis's identity voice. `TruthValidator` verifies `correct_identity`, `correct_creator`, `relationship_clear`. Response logged to `ms_jarvis_memory` ChromaDB collection (self-heals from 50 docs organically; see Chapter 12 §12.3 note on consciousness bridge line 74) and `episodic_log`.
...
`ms_jarvis_memory`: 50 docs (rebuilt April 16); self-heals organically via consciousness bridge (line 74 of `msjarvisconsciousnessbridge.py`) on every session turn — will return to 296+ naturally. `appalachian_cultural_intelligence`: 890 docs; threshold ≥563 — will exceed on next ACI pipeline run.
...
- **Judge pipeline.** Consensus answer only — never raw_responses dump. All 5 judge containers in `docker-compose.yml` with `build: context: ./services, dockerfile: Dockerfile.judge` and `restart: unless-stopped`. `judge_to_pituitary_bridge.py` must be volume-mounted into `jarvis-judge-pipeline`. Ghost containers on `container_name:` slot block volume mounts — clear orphans before recreating. ML-DSA-65 keys: back up `judge-keys/judge_sk.bin` (4032 bytes) to offline storage.
- **ChromaDB.** Host port **8002** (v2 API). Container-to-container: `jarvis-chroma:8000`. v1 path deprecated. Required model: `all-minilm:latest` (384-dim). **48 collections, 6,740,034 vectors** (April 16). `ms_jarvis_memory`: 50 docs — self-heals via consciousness bridge line 74. `appalachian_cultural_intelligence`: 890 docs, threshold ≥563. Preflight thresholds use `-ge` not `-eq`.
- **msjarvis-db.** Port 5433, bound to `127.0.0.1` (corrected April 16 — orphaned container removed, recreated via compose). GBIM entity store is **ChromaDB `gbim_worldview_entities`** (5,416,521 vectors), not a relational table.
- **msjarvisgis.** Port 5432, **45 GB, 551 tables** (April 16 confirmed).
...
- **EEG stack.** `Dockerfile.eeg` single image for all three EEG services. `eeg_shared/pia_state.py` reads `JARVIS_PIA_STATUS_URL` from env. EEG blocks must be inserted before `networks:` key in `docker-compose.yml` — structural fix (April 16). Preflight gate includes EEG health checks. Backups in `~/msjarvis-repair-backups/`.
...
- **Compose management.** All 109 containers defined in `~/msjarvis-rebuild-working/msjarvis-rebuild/docker-compose.yml`. Docker Compose v5.1.0. ★ 109 containers confirmed Up, zero Restarting, zero Exited — April 16, 2026. Backups saved to `~/msjarvis-repair-backups/`.
...
| *(March 28 remediation)* | 2026-03-28 | ★ Security remediation: `jarvis-i-containers` + `jarvis-memory` → 127.0.0.1; `_auth()` 4 routes; `JARVIS_API_KEY` set; 96/96 Up; ChromaDB 40 col / 6,675,442 vec; `spiritual_rag` deduped −19,338; `psychological_rag` restored 968 docs; `msjarvis` 5433 restored; `msjarvisgis` 5432 confirmed |
...
**Chapter 11 is CLOSED. April 16, 2026 production state confirmed: 109/109 containers Up, all 27 preflight gates ✅ green, GPU pipeline 99–115s (102.58s confirmed), BBB_OUTPUT_BLOCKING=true, rag_grounded_v2 + llm_judge_v3 active, heuristic_contradiction_v1 retired, ML-DSA-65 FIPS 204 signing across all 5 judge containers, judge_to_pituitary_bridge.py mounted, nbb_pituitary_gland live at 127.0.0.1:8108 mode elevated, nbb_mother_carrie_protocols live at 127.0.0.1:8107 with 5 protocols and MOTHER_CARRIE_KIDD anchor, EEG Delta/Theta/Beta all active on qualia-net with PIA OK, ChromaDB v2 48 collections / 6,740,034 vectors (host port 8002), ms_jarvis_memory rebuilt and self-healing, msjarvis-db corrected to 127.0.0.1, zero 0.0.0.0 exposures system-wide.**

---

*Last updated: 2026-04-16 — Carrie Kidd (Mamma Kidd), Mount Hope WV*

*★ March 27, 2026: Heading restructured; Figure 11.1 annotated — consciousness-bridge port note per Ch. 12 §12.1; §11.3 consciousness bridge parenthetical added; §11.7 ChromaDB host/container port distinction added; autonomous learner AAPCAppE scraper sprint update added.*

*★ March 28, 2026: Security remediation complete — 96/96 Up; `jarvis-i-containers` + `jarvis-memory` → 127.0.0.1; `_auth()` 4 routes; `JARVIS_API_KEY` set. Container count 80 → 96 throughout. ChromaDB full audit: 40 col / 6,675,442 vec; `spiritual_rag` deduped; `psychological_rag` restored. `msjarvis` 5433 restored; `msjarvisgis` 5432 confirmed. `jarvis-memory` §11.3 entry added. March 28 git milestone added. Security hardening section expanded.*

*★ April 16, 2026 — CHAPTER CLOSED: Container count 96 → 109. GPU pipeline 99–115s / 102.58s confirmed; CPU 436s baseline RETIRED STALE. `BBB_OUTPUT_BLOCKING=true` since April 6; `rag_grounded_v2` + `llm_judge_v3` active; `heuristic_contradiction_v1` retired. ML-DSA-65 FIPS 204 all 5 judge containers; sig=3309 bytes; verify=True; `judge_to_pituitary_bridge.py` mounted (ghost container 84207ec4fca7 cleared). `nbb_pituitary_gland` 127.0.0.1:8108 mode elevated — upstream governance gate. `nbb_mother_carrie_protocols` 127.0.0.1:8107; 5 protocols; MOTHER_CARRIE_KIDD anchor. `nbb_qualia_engine` host 8303 → 7008/tcp. EEG Delta (8073) / Theta (8074) / Beta (8075) all deployed on qualia-net; PIA OK; 16 BBB events. ChromaDB v2 updated: 48 col / 6,740,034 vec; host port 8002; v1 deprecated. `msjarvis-db` 5433 corrected 0.0.0.0 → 127.0.0.1 (April 16 orphaned container fix). `ms_jarvis_memory` rebuilt 50 docs (self-heals via consciousness bridge line 74). `appalachian_cultural_intelligence` threshold → ≥563. Preflight thresholds corrected to -ge throughout. `msjarvis_docs` updated to 7,465 items. Community resources: 64 verified, all 55 WV counties. `msjarvisgis` updated to 45 GB / 551 tables. BBB output filter facade at host 8017 documented. Qualia engine internal-only binding documented. Figure 11.1 and status table fully updated. April 16 git milestone added. docker-compose.yml structural corruption resolved (EEG before networks: key). Backups confirmed in ~/msjarvis-repair-backups/. Chapter 11 CLOSED.*
