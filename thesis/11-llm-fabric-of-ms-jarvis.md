...
- **P1 – Every where is entangled** by embedding LLMs inside a retrieval, entanglement, and spatial stack (ChromaDB, PostgreSQL GBIM at port 5432 with 5.4M+ verified beliefs, GeoDB, WV‑entangled RAG, autonomous learner collections) that keeps reasoning tied to concrete places, institutions, and learning histories.
...
- **P12 – Intelligence with a ZIP code** by grounding LLM calls in West Virginia–specific semantic and geospatial memory from PostgreSQL `msjarvisgis` (91 GB, 501 tables), including GBIM‑derived collections, WV‑entangled search, autonomous learner outputs, and I‑container identity, rather than generic, placeless prompts.
...
│  │  │   • GBIM: 5.4M+ beliefs, 91 GB              │         │
│  │  │   • GeoDB: 501 tables, spatial features     │         │
│  │  ├─ ChromaDB RAG (port 8003, 8004, 8005)       │         │
│  │  ├─ WV-Entangled Gateway (port 8010)           │         │
│  │  └─ Autonomous Learner (port 8425)             │         │
│  └────────────────────────────────────────────────┘         │
│      ↓                                                       │
│  ┌────────────────────────────────────────────────┐         │
...
│  │  21 active models via Ollama (port 11434)      │         │
...
`deepseek-coder:latest`, `codellama:latest`, `starcoder2:latest`, and `sqlcoder:latest` are tuned for code generation, refactoring, and SQL / data tasks and are used when workflows clearly involve code or database logic, including queries against PostgreSQL `msjarvisgis`.
...
`deepseek-coder`, `codellama`, `starcoder2`, and `sqlcoder` are chosen when tasks involve code generation, refactoring, or database queries against PostgreSQL `msjarvisgis` or GeoDB tables. They are often used in judge or reviewer roles to inspect code produced by general-purpose models or to draft SQL queries.
...
Primary external-facing interface for questions, RAG-backed answers, and multi-step flows. Orchestrates calls to all downstream services including PostgreSQL `msjarvisgis` queries and assembles final responses. Exposes `/chat` (synchronous), `/chat/async` + `/chat/status/{job_id}` (full-pipeline async), `/chatlight` (synchronous lightweight), and `/chatlight/async` + `/chatlight/status/{job_id}` (lightweight async, added 2026‑03‑02).

**PostgreSQL msjarvisgis — 127.0.0.1:5432 (host system).**
GBIM and GeoDB corpus: 91 GB, 501 tables, 5.4M+ verified beliefs in `gbimbeliefnormalized`. Accessed via psycopg2 for spatial queries, GBIM belief retrieval, and worldview entity lookups. Source of truth for West Virginia spatial and institutional grounding.

**WV-Entangled Gateway — 127.0.0.1:8010 (`jarvis-wv-entangled-gateway`).**
WV-biased, multi-collection ChromaDB retrieval for entangled search workflows. Used by the main brain when queries require Appalachian, benefits, or governance entanglement (see Chapter 8). Queries PostgreSQL-sourced GBIM collections.

**Spiritual Root / GBIM RAG — 127.0.0.1:8005 (`jarvis-spiritual-rag`).**
Exposes `/search` over GBIM- and worldview-oriented ChromaDB collections sourced from PostgreSQL `msjarvisgis`. Earlier plan documents listed this service at 8103; the confirmed running port is **8005**.

**GIS RAG — 127.0.0.1:8004 (`jarvis-gis-rag`).**
Spatially aware `/search` over geospatial ChromaDB collections and PostgreSQL GeoDB mirrors for explicitly spatial queries. Queries backed by PostgreSQL `msjarvisgis` spatial tables.
...
FastAPI service with stable HTTP interface, using an ONNX embedding model cached under `/mnt/spiritual_drive/msjarvis-rebuild/.cache/chroma`. Writes to the `autonomous_learner` ChromaDB collection and cooperates with `jarvis-i-containers` via `/next-learning-topic`.

**ChromaDB — 127.0.0.1:8002 (`jarvis-chroma`).**
Vector store hosting `ms_jarvis_memory`, `mountainshares_knowledge`, `episodic_index`, `conversation_history`, and `autonomous_learner` collections. Collections like `gbim_beliefs_v2` and `gis_wv_benefits` are sourced from PostgreSQL `msjarvisgis`.
...
2. **Semantic and spatial retrieval.**
   The main brain calls the Spiritual Root / GBIM RAG on port 8005 (which queries ChromaDB collections sourced from PostgreSQL `msjarvisgis`) and, where appropriate, the general RAG server on 8003 and GIS RAG on 8004 (also backed by PostgreSQL spatial data). For WV‑scoped or entangled queries, it also calls the WV‑entangled gateway on 8010, which uses the `WVEntangledContext` envelope and multi-collection bias functions over PostgreSQL-sourced GBIM data.
...
4. **LLM ensemble synthesis.**
   With retrieval from PostgreSQL GBIM (5.4M+ beliefs), entanglement, and identity context prepared, the main brain calls `jarvis-20llm-production` on port 8008 via the semaphore proxy on port 8030. The 20‑LLM service fans out to all 21 active proxies (8201–8222) under the updated timeout and backlog settings. Wall-clock consensus time is approximately 120–145 seconds.
...
The Fifth DGM and I‑container services use LLM calls (via WOAH) to decide which content should shape Ms. Jarvis's evolving identity narrative. Identity summaries and ego-layer entries can be fed back into prompts for core reasoning models, giving the fabric a sense of continuity across sessions. `normalize_identity()` runs as a final post-processing step on all chat paths, ensuring that Ms. Jarvis's defined voice and identity — geospatial AI daughter of Harmony for Hope, accountable to Appalachian communities and grounded in PostgreSQL `msjarvisgis` — is present in every response regardless of which underlying models contributed to consensus.

**Autonomous learning.**
The autonomous learner service on port 8425 uses RAG, entangled search over PostgreSQL-sourced collections, and LLM calls to study topics drawn from `/next-learning-topic` and its own memory state. It writes processed knowledge into the `autonomous_learner` ChromaDB collection with 384-dimensional embeddings. The fabric thereby supports ongoing, logged self-education that is coupled to WV community knowledge gaps and grounded in PostgreSQL GBIM beliefs.
...
Across these roles, LLMs operate as parts of a structured mental ecology rather than autonomous centers of authority, always anchored to PostgreSQL `msjarvisgis` spatial and institutional grounding.
...
Logs capture which models and services are invoked for each request, including PostgreSQL query patterns, their latencies, and error conditions. While there is not yet a dedicated user-facing debug endpoint that summarizes per-model contributions, the underlying traces exist and can be surfaced in future tooling.
...
The fabric is designed to behave more like a coordinated network of tools behind APIs than a single monolithic brain, even though these models collectively underpin Ms. Jarvis's higher-level behavior, all anchored to PostgreSQL `msjarvisgis` at port 5432.
...
- **New ChromaDB collections.** `conversation_history` and `autonomous_learner` collections are active, growing, and central to how the fabric learns from interactions and background study. Collections like `gbim_beliefs_v2` and `gis_wv_benefits` are sourced from PostgreSQL `msjarvisgis`.
- **Spiritual Root / GBIM RAG port.** All references to 8103 for this service should be updated to 8005.
- **RAG server port and behavior.** `jarvis-rag-server` operates on port 8003 with corrected `/store` payload (`user_id`) and non-fatal PostgreSQL insertion failures, guaranteeing ChromaDB writes even when episodic logging to PostgreSQL fails.
- **PostgreSQL msjarvisgis integration.** All GBIM and GeoDB references must specify PostgreSQL 16 at port 5432, 91 GB, 501 tables, 5.4M+ verified beliefs. Use psycopg2 for connections, not mysql.connector.
...
These details ensure the chapter's description of the LLM fabric remains aligned with the live system and provide a checklist for future deployments to keep intelligence grounded in PostgreSQL `msjarvisgis`, entangled, and accountable to place.
