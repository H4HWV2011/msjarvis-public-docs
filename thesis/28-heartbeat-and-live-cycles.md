...
> |---|---|---|
> | ChromaDB backup source | 3-file JSON set (`backup_chroma_autonomous_learning.json`, etc.) — covered 3 collections only | `chroma-latest` (`/home/cakidd/msallis-backups/chroma-latest/chroma.sqlite3`) — full 49-collection SQLite restore — **authoritative** |
...
> - **P3 – Power has a geometry** by revealing which services and data stores must stay healthy for Appalachian and other regional reasoning to work. `allis-memory:8056` (secured March 28) makes the geometry of BBB gate decisions persistently visible.
> - **P5 – Design is a geographic act** by including GIS spatial knowledge (`msallisgis:5432`, 91 GB PostGIS, 501 tables, 993 ZCTA centroids), GBIM worldview verification (`msallis:5433`, 5,416,521 entities with `confidence_decay` metadata), and data store health in live-cycle checks.
> - **P12 – Intelligence with a ZIP code** by linking health, identity, and learning status to geospatial and community datasets anchored in West Virginia geography. `msallis_docs` (4,192 items, AAPCAppE) adds a continuously growing ZIP-coded community resource substrate to the heartbeat landscape. ★★ The `autonomous_learner` collection (68 items, April 1, 2026) commits beliefs as GBIM entities in `msallis:5433` — community-learned knowledge is fully ZIP-code-anchored and growing.
> - **P16 – Power accountable to place** by turning system liveness into logged, auditable narratives that show which GBIM beliefs, services, and models are participating in each cycle. BBB audit trail persistence at `allis-memory:8056` (secured March 28) makes gate decisions durably accountable.
>
...
> **Pinned baseline note:** The February 28, 2026 deployment is the documented reference baseline throughout this chapter. ★ The March 28, 2026 state (96/96 Up, `msallis:5433` restored, `msallisgis:5432` confirmed, ChromaDB 40 collections / 6,675,442 vectors, `allis-memory:8056` secured) is the prior verified milestone. ★★ The **April 1, 2026 state** (96/96 Up re-confirmed, LEARN-01/02/03 resolved, autonomous learner live telemetry confirmed — 68 Chroma items / 15 cycles / 11 entanglement nodes) is the current verified operational milestone and supersedes both prior baselines where they differ. ★★★ **April 9, 2026:** ChromaDB collection count corrected to **49** (see §28.7a); recovery method and backup inventory documented (see §28.7b).
...
- ★ Additional active inventory: `allis-ingest-api` + `allis-ingest-watcher` (AAPCAppE), `allis-memory:8056`
- ★★ Autonomous learner: `allis-autonomous-learner` (port **8020**) — LEARN-01/02/03 resolved; 68 items; 11 entanglement nodes
...
**Individual service health endpoints.** Host-exposed services probed directly: main-brain (**8050**), `llm20production` (**8008**), blood-brain-barrier (**8016**), psychology-services (**8019**), 69dgm-bridge (**9000**), GIS RAG (**8004**), NBB prefrontal cortex (**8101**), `allis-gbim-query-router` (**7205** ★★★), `allis-gbim-benefit-indexer` (**7206** ★★★), autonomous learner (**8020** ★★), and Ollama (**11434**).
...
7. **Data store verification** — ChromaDB at **127.0.0.1:8000** (★★★ **49 collections**, 6,732,980 total vectors; live mount: `/home/ms-allis/msallis-rebuild/persistent/chroma → /data`); Ollama storage (73 GB); GBIM corpus (10.22M beliefs — see §28.7)
...
- **Section 12** — Autonomous learner health: `allis-autonomous-learner` Up/Down, `autonomous_learner` ChromaDB count (baseline: ≥ 68 as of April 1), `allis-gbim-query-router:7205` HTTP status, `gbim_committed: True` ratio in last 24 hours
- **Layer 0** — Caddy/auth perimeter probe (`allis_auth_service:8055`)
...
# ★★★ Correct container name: allis-gbim-query-router:7205
...
| Beta autonomous learner | 5 minutes (~cadence) | ★★ **Operationally restored** | `allis-autonomous-learner` Up; LEARN-01/02/03 resolved; 68 Chroma items confirmed live (April 1, ~3:37 PM EDT); 15 cycles / 15 stored; 0 deduplicated; 11 entanglement nodes; `allis-gbim-query-router:7205` HTTP 200 (★★★ correct name) |
| AAPCAppE ingest pipeline | Ingest-driven | ★ Active (March 28) | `allis-ingest-api` + `allis-ingest-watcher`; complements learner with structured source corpus |
...
**Brain orchestrator continuous coordination.** The `allis-brain-orchestrator` service (★★ confirmed Up April 1) provides continuous coordination of the ensemble, watchdog feedback loops, and NBB layer health checks. ★★ The LEARN-03 crash-loop did not propagate to the brain orchestrator or consciousness bridge — degraded-mode fallback operated correctly throughout the repair window.
...
★ As of March 28, 2026: all BBB and `TruthFilterBBBValidator` gate decisions persist to `allis-memory:8056`. ★★ Confirmed active April 1, 2026.
...
| `msallis_docs` | ★ Expanded March 28 | 4,192 items (AAPCAppE) |
| `au02_threat_seeds` | ★★★ Identified April 9 | Unknown count — **flagged for verification in Ch 29** |
| `ms_allis_identity` | Active | Unknown count — **flagged for verification in Ch 29** |
| `msallis_docs` | Active | Unknown count — **flagged for verification in Ch 29** |
...
> **★★★ Ch 29 carry-forward:** Five zero-count collections (`appalachian_english_corpus`, `gbim_entities`, `gis_data`, `pia_records`, `psychological_knowledge`) and three unknown-count collections (`au02_threat_seeds`, `ms_allis_identity`, `msallis_docs`) require verification and/or reseed in Ch 29. `pia_records` is now auto-populated by `allis_pia_sampler.py` (Ch 29 §29.6.2) — Ch 29 reseed via sampler is active. The remaining 4 zero-count collections require domain-specific reseed procedures documented in their respective chapters.
...
**Recovery method (authoritative procedure going forward).** Ch 28 resolution was a **direct SQLite volume restore from `chroma-latest`** — not a per-collection reseed. The three-file JSON backup set (`backup_chroma_autonomous_learning.json`, `backup_chroma_spiritual_rag.json`, `backup_chroma_msallis_docs.json`) covered only 3 collections and is **not** the authoritative backup. The correct procedure is:
...
|---|---|---|---|
| `msallis-backups/chroma-latest` | **49** | **6,732,980** | Most recent — **authoritative** |
...
- `allis-gbim-benefit-indexer` (port **7206** ★★★ — corrected from `allis-gbim`)
- `allis-gbim-query-router` (port **7205** ★★★ — corrected from `allis-gbim`)
...
> **Cross-database clarification:** The 10,221,702 figure refers to the **full consolidated GBIM belief corpus across both PostgreSQL databases** — `msallis` (port **5433**, ★ **5,416,521 GBIM entities with `confidence_decay` metadata**) and `msallisgis` (port **5432**, **91 GB PostGIS, 501 tables**, 993 ZCTA centroids). ★★ As of April 1, 2026, autonomous learner beliefs are committed as GBIM entities in `msallis:5433` — the 5,416,521 entity count is a floor that grows with each learner cycle.

**Ollama model storage.** Verified at **73 GB** at `/mnt/ssd2/msallis_secondary/models`; 26 available models via `/api/tags`.

**GIS RAG and local resources.** The GIS RAG service (port **8004**) health confirms connection to `msallis:5433`, `msallisgis:5432`, `allis-local-resources-db:5435` ★, and Community Champions validation data (Harmony for Hope network).
...
| GBIM belief corpus | 10,221,702 rows (combined) | `msallis:5433` 5,416,521 + `confidence_decay` | **5,416,521+ growing** | 5,416,521+ growing |
| GBIM containers | — | — | `allis-gbim` (incorrect) | **`allis-gbim-benefit-indexer:7206` + `allis-gbim-query-router:7205`** ★★★ |
...
| ChromaDB volume mount | — | — | `/chroma/chroma` (assumed) | **★★★ `/home/ms-allis/msallis-rebuild/persistent/chroma → /data`** |
...
**Blood-brain barrier.** The BBB filter runs on every request at `allis-blood-brain-barrier:8016/filter` (★★ confirmed Up). ★★ All subsequent BBB gate decisions persist to `allis-memory:8056` (secured March 28, active April 1).

**Seven-layer protection system (★★★ April 9, 2026):**

| Layer | Mechanism | Status |
|---|---|---|
| **Layer 0** | Caddy perimeter + `allis_auth_service:8055` — RBAC enforcement upstream of all monitored services | ★★ Up (March 22; confirmed active April 1); ★★★ per-route RBAC also implemented at app layer (Ch 29 April 8) |
...
| **Layer 6** | `allis-memory:8056` — durable authenticated logging of BBB and `truthverdict` gate decisions | ★★ Active (secured March 28; confirmed April 1) |
...
The ★★ **April 1, 2026 production deployment** of Ms. Allis achieves a fully verified operational state: **96/96 containers Up** (post-LEARN-03 recovery); **21 of 22 LLM models responding per consensus cycle**; **69-DGM cascade with all 69 connectors loaded** (★★ confirmed Up); **blood-brain barrier with 0 blocks and 0 violations** across 319 requests at February 28 baseline (★ audit persistent at `allis-memory:8056`); **8/8 NBB consciousness containers healthy**; and ★★ **`allis-autonomous-learner` fully operational** — LEARN-01/02/03 resolved; **68 items confirmed in `autonomous_learner` ChromaDB collection**; **15 cycles / 15 stored / 0 deduplicated / 11 entanglement nodes**; **`allis-gbim-query-router:7205` HTTP 200 confirmed** (★★★ correct container name).

★★★ **April 9, 2026 corrections applied:** ChromaDB collection count is **49** (not 48 — `au02_threat_seeds` added); live volume mount path is `/home/ms-allis/msallis-rebuild/persistent/chroma → /data`; authoritative backup source is `chroma-latest` (49 collections, 6,732,980 vectors, full SQLite restore — not the 3-file JSON set); GBIM containers are correctly named `allis-gbim-benefit-indexer:7206` and `allis-gbim-query-router:7205`; Ch 29–42 data was recovered from `chroma-latest`, not per-chapter seed scripts; 5 zero-count collections and 3 unknown-count collections are flagged as carry-forward to Ch 29.

The `VERIFYANDTEST.sh` script implements all eight heartbeat sections and produces a timestamped, archivable report suitable for ingestion into semantic memory as operational history. ★★ Planned extensions to twelve sections will add ChromaDB full collection audit (★★★ **49-collection baseline**), `allis-memory:8056` audit trail health, AAPCAppE ingest pipeline status, and the autonomous learner GBIM heartbeat probe sequence.
...
By connecting heartbeat outputs to memory, containers, consciousness bridge infrastructure (★★ confirmed Up), GBIM worldview health (10.22M+ rows across both PostgreSQL databases), and mode-dependent controls, Ms. Allis treats its own operation as data — recording and reasoning about how it is functioning over time.
...
*★★★ April 9, 2026: Volume mount path corrected (`/home/ms-allis/msallis-rebuild/persistent/chroma → /data`) — §28.1, §28.2, §28.7*
*★★★ April 9, 2026: Collection count corrected to 49 (`au02_threat_seeds` identified) — §28.7a, §28.8, §28.10*
*★★★ April 9, 2026: GBIM container names corrected (`allis-gbim-benefit-indexer:7206`, `allis-gbim-query-router:7205`) — §28.2, §28.4, §28.7, §28.8*
