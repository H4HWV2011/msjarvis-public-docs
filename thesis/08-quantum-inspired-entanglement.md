...
> **★ Security posture — March 28, 2026:** All services including `jarvis-wv-entangled-gateway` (port 8010) are bound to `127.0.0.1`. The `0.0.0.0` exposures on `jarvis-i-containers` (8015) and `jarvis-memory` (8056) were corrected during the March 28 remediation sprint. `_auth()` is confirmed present on all 4 sensitive routes in `ms_jarvis_memory.py`. `JARVIS_API_KEY` env var confirmed set. **96/96 containers Up — zero Restarting, zero Exited.**

> **★ ChromaDB full audit — March 28, 2026:** 40 active collections, 6,675,442 total vectors. `spiritual_rag` deduplicated (−19,338 duplicate vectors). `psychological_rag` restored (968 docs). `msjarvis_docs` expanded (4,192 items). `appalachian_cultural_intelligence` (820 items) and `aaacpe_corpus` (65 docs) confirmed as two distinct collections. `msjarvis` port 5433 restored. All services bound to `127.0.0.1`.
...
When an important anchor is updated — such as a governance rule, a spatial boundary in PostgreSQL PostGIS (`msjarvisgis`, port 5432), a benefit‑program constraint, or a key norm — the design intends that Ms. Jarvis will apply an entanglement update to the correlated set rather than only mutating the single item.
...
\]
which is large when \(x_j\) carries many of the anchor's critical tags. More sophisticated schemes could incorporate semantic distance, spatial distance (for PostgreSQL GeoDB features at `msjarvisgis` port 5432), or importance weights.
...
The server issues queries to multiple ChromaDB collections at once. The canonical production collection names (per Ch 7 §7.2.1 confirmed live collection list — ★ updated March 28, 2026) are:

- `gbim_worldview_entities` (5,416,521 entities — governance, norms, and institutional rules with beliefs sourced from PostgreSQL `msjarvisgis` port 5432)
- `gis_wv_benefits` (WV benefits facilities — spatial and geospatial descriptions derived from PostgreSQL GeoDB)
- `gbim_beliefs_v2` (GBIM beliefs v2 — additional spatial entities and features from PostgreSQL `msjarvisgis`)
...
- `appalachian_cultural_intelligence` (★ 820 items — confirmed live; distinct from `aaacpe_corpus`)
- `aaacpe_corpus` (★ 65 docs — `jarvis-aaacpe-scraper` port 8033, 39 sources; distinct from `appalachian_cultural_intelligence`)
- `spiritual_rag` (deduplicated — ★ 19,338 duplicate vectors removed March 28, 2026)
- Additional collections as configured.

> **⚠️ Collection Name Reconciliation Note:** Earlier drafts of this chapter referenced collection names `GBIM`, `gisgeodata`, `benefit_programs`, `GeoDB`, and `utility_enrollments`. These names do not appear in the confirmed live collection list from Ch 7 §7.2.1 (★ updated March 28, 2026 full audit: 40 collections, 6,675,442 vectors). If `jarvis-wv-entangled-gateway`'s internal configuration maps these as service-layer aliases to the canonical ChromaDB collection names above, that alias mapping should be documented explicitly in the service's configuration file. If they were placeholder names from an earlier design, they should be replaced throughout with the canonical names listed above. Until the alias mapping is confirmed in the service config, use the canonical collection names from Ch 7 §7.2.1 in all documentation and client code.
...
> All embedding calls from this service use `all-minilm:latest` (384-dim, cosine). PostgreSQL source is `msjarvisgis` at port 5432 (★ corrected March 28, 2026 — was: 5452 in earlier drafts).
...
```python
# Autonomous learner — per-cycle entanglement update hook (current scaffold)
import json
...
def update_entanglement(
    ...
    """
    Current scaffold: increment edge weight between current_topic
    and each co-retrieved topic. Future work: replace with
    principled co-retrieval measure from WV-entangled RAG traces.
    """
...
Documents in ChromaDB collections queried at port 8010 — including `gbim_worldview_entities`, `gbim_beliefs_v2`, `gis_wv_benefits`, `geospatialfeatures`, `GBIM_Fayette_sample`, `governance_rag`, `commons_rag`, `appalachian_cultural_intelligence`, and `aaacpe_corpus` — are increasingly annotated with tags and metadata fields that reference GBIM entities from PostgreSQL `msjarvisgis` (port 5432 — ★ corrected March 28, 2026), GBIM‑derived spatial collections, GeoDB features, counties, and states. These fields form the backbone of entangled sets and are what the WV‑biased retrieval function inspects when applying coupling strengths.
...
**Four-database alignment — ★ confirmed March 28, 2026:**

```python
# Port reference confirmed March 28, 2026
# msjarvis (GBIM beliefs) — port 5433 (★ restored March 28)
# msjarvisgis (PostGIS spatial) — port 5432 (★ corrected March 28 — was: 5452)
# jarvis-local-resources-db — port 5435
# ChromaDB — host port 8002, container port 8000
# jarvis-wv-entangled-gateway — port 8010

DATABASE_PORTS = {
    "msjarvis": 5433,       # GBIM beliefs — ★ restored March 28, 2026
    "msjarvisgis": 5432,    # PostGIS spatial — ★ corrected March 28, 2026
    "local_resources": 5435,
    "chroma_host": 8002,
    "chroma_container": 8000,
...
**Concrete implementation in the current deployment (concrete layer) — ★ updated March 28, 2026.**
A formal JSON schema and Pydantic model for `WVEntangledContext`, enforced in the running RAG server at **port 8010** (confirmed ✅ Running, bound to `127.0.0.1`). A WV‑biased, multi‑collection retrieval path that uses the entanglement envelope to modulate scores across ChromaDB collections — including:
- `gbim_beliefs_v2` sourced from PostgreSQL `msjarvisgis` (port 5432 — ★ corrected March 28) with 5.4M+ verified beliefs
- `geospatialfeatures` at 60,000 items (confirmed live March 26, 2026)
- `GBIM_Fayette_sample` at 1,535 items (confirmed live March 26, 2026)
- `governance_rag` at 643 chunks (confirmed live March 26, 2026)
- `commons_rag` at 306 chunks (confirmed live March 26, 2026)
- `appalachian_cultural_intelligence` at 820 items (confirmed distinct from `aaacpe_corpus`)
- `aaacpe_corpus` at 65 docs (confirmed distinct from `appalachian_cultural_intelligence`)
- `spiritual_rag` (deduplicated — ★ 19,338 duplicate vectors removed March 28, 2026)
- `psychological_rag` at 968 items (★ restored March 28, 2026)

All ChromaDB queries from this service use **`all-minilm:latest` (384-dim, `hnsw:space: cosine`)** and the **ChromaDB v2 API** (`/api/v1/` returns HTTP 410 Gone as of March 22, 2026). The service logs both pre‑bias and post‑bias rankings for inspection.

A JSON‑backed topic graph maintained by the autonomous learner (`jarvis-autonomous-learner`), updated at the end of each learning cycle, providing a persistent record of inferred topic entanglements. The `autonomous_learner` collection holds **21,181+ items as of March 18, 2026, growing ~288/day**.

**★ March 28, 2026 full audit: 40 active collections, 6,675,442 total vectors across all ChromaDB collections.**
...
- Enriched ChromaDB metadata coverage so that WV‑aligned documents — especially those tied to GBIM entities represented in PostgreSQL `gbimbeliefnormalized` (`msjarvisgis` port 5432) and mirrored in `gbim_beliefs_v2`, `gis_wv_benefits`, `GBIM_Fayette_sample`, and `geospatialfeatures` — are reliably boosted in entangled retrieval traces (OI-19).
