...
**Last updated: April 1, 2026 — OI-22 closed (air-gapped USB copy confirmed at `/media/cakidd/writable/`, commit `3218392e`); OI-30 closed (Gate 30 passing, MountainShares/DAO smoke tests confirmed); OI-31 closed (EEG delta/theta/beta pipeline documented, §42.13); OI-36-A closed (Caddy `forward_auth` live, 401 confirmed, commit `f2e93422`); OI-CRYPTO-VT closed (Gate 29 passing, `jarvis-crypto-policy:8099` wired into `VERIFYANDTEST.sh`); VERIFYANDTEST run April 1 — FAIL=0, WARN=0 after duplicate line removal; all prior March 22–28 corrections carry forward unchanged.**
...
> - All 5 judge ML-DSA-65 signing keys remained active throughout the March 22–25 sprint. The judge pipeline continued passing 19/19 checks on every validation run. No key rotation events, no signing failures, no judge container restarts affecting key availability.  
> - GPG backup `judge_sk_backup_20260322.gpg` confirmed on-machine only during this sprint. The file remains at `~/judge-sk-backup-20260322.gpg` on the Legion 5 host.  
...
> - Preflight gate targeting 28 PASS / 0 FAIL — four new gates identified (gates 25–28): `jarvis-memory:8056` auth/logging, `confidence_decay` presence, ChromaDB ≥ 40 collections, `psychological_rag` ≥ 968 docs. Script update pending.  
> - Output BBB Phase 4.5 decisions now logged to `jarvis-memory:8056` — log/passthrough mode unchanged; calibration data now accumulating.  
...
> - **OI-22 CLOSED** — air-gapped USB copy of `judge_sk_backup_20260322.gpg` confirmed at `/media/cakidd/writable/`, commit `3218392e`. See §42.12.  
...
> - **Gate 26 corrected** — now targets `msjarvisgis` on `jarvis-local-resources-db:5435`, `memories` table created April 1, 2026 with `confidence_decay` confirmed non-null; OI-13 closed.
...
| Partially implemented / scaffolded | Signature verification at the BBB layer — BBB receives signed verdicts but does not yet extract and verify the Dilithium signature before processing the verdict dict (Phase 2 BBB upgrade; see Chapter 16 §16.9). Key rotation procedure — `judge_sk.bin` / `judge_pk.bin` are static keys; no automated rotation schedule. Full AU-02 regex-based authority impersonation detection remains future work. |
...
`dilithium_py` provides a pure-Python implementation of ML-DSA-65 (CRYSTALS-Dilithium) and is installed in the judge image via a `pip install` step:
...
- Phase 4.5 (BBB output gate, log/passthrough mode): every flagged-but-passed decision logged for calibration.  
...
for container in jarvis-judge-pipeline jarvis-judge-truth \
  ...
  docker exec "$container" python3 -c \
    "import hashlib; pk=open('judge_pk.bin','rb').read(); print(hashlib.sha3_256(pk).hexdigest()[:16])"
done
```
...
Gate 29 check (now wired into `VERIFYANDTEST.sh`):
...
- Wiring Dilithium signature verification into BBB's `output_guard` endpoint (Phase 2 BBB upgrade; see Chapter 16 §16.9).  
- Adding signed audit log entries for all judge verdicts.  
- Implementing key rotation tooling and a documented annual cadence.  
- Exposing an external verification endpoint for `judge_pk.bin` to support community and academic review.  
- Adding embedding-based semantic AU-02 detection beyond string-match guards (v2).  
- Output BBB recalibration — threshold tuning to eliminate maternal/community-voice false positives.  
